defmodule GeoguessrPartyWeb.GeoguessrController do
  use GeoguessrPartyWeb, :controller

  alias GeoguessrParty.Game
  alias GeoguessrParty.Game.Party

  action_fallback GeoguessrPartyWeb.FallbackController

  # def index(conn, _params) do
  #   parties = Game.list_parties()
  #   render(conn, "index.json", parties: parties)
  # end

  # def create(conn, %{"party" => party_params}) do
  #   with {:ok, %Party{} = party} <- Game.create_party(party_params) do
  #     conn
  #     |> put_status(:created)
  #     |> put_resp_header("location", Routes.party_path(conn, :show, party))
  #     |> render("show.json", party: party)
  #   end
  # end

  def show(conn, %{"id" => id}) do
    party = Game.get_party_by_geoguessr_id!(id)
    render(conn, "show.json", party: party)
  end

  # def update(conn, %{"id" => id, "party" => party_params}) do
  #   party = Game.get_party!(id)

  #   with {:ok, %Party{} = party} <- Game.update_party(party, party_params) do
  #     render(conn, "show.json", party: party)
  #   end
  # end

  # def delete(conn, %{"id" => id}) do
  #   party = Game.get_party!(id)

  #   with {:ok, %Party{}} <- Game.delete_party(party) do
  #     send_resp(conn, :no_content, "")
  #   end
  # end

  # def show_by_geoguessr_id
end
